...
def plot_waveform(waveform, sr, title="Waveform", ax=None,file_name='images/waveform.png'):
...
OTHER_VOICE_DIR='other_voice'

## IMAGE Directories
if not os.path.exists(IMAGE_DIR) and  not os.path.isdir(IMAGE_DIR):
    os.mkdir(IMAGE_DIR)
if not os.path.exists(f'{IMAGE_DIR}/{TARGET_VOICE_DIR}') and  not os.path.isdir(f'{IMAGE_DIR}/{TARGET_VOICE_DIR}'):
    os.mkdir(f'{IMAGE_DIR}/{TARGET_VOICE_DIR}')
if not os.path.exists(f'{IMAGE_DIR}/{OTHER_VOICE_DIR}') and  not os.path.isdir(f'{IMAGE_DIR}/{OTHER_VOICE_DIR}'):
...
if not os.path.exists(f'{WAVEFORM_DIR}/{TARGET_VOICE_DIR}') and  not os.path.isdir(f'{WAVEFORM_DIR}/{TARGET_VOICE_DIR}'):
    os.mkdir(f'{WAVEFORM_DIR}/{TARGET_VOICE_DIR}')
if not os.path.exists(f'{WAVEFORM_DIR}/{OTHER_VOICE_DIR}') and  not os.path.isdir(f'{WAVEFORM_DIR}/{OTHER_VOICE_DIR}'):
...
if not os.path.exists(f'{APP_IMAGE_DIR}/{TARGET_VOICE_DIR}') and  not os.path.isdir(f'{APP_IMAGE_DIR}/{TARGET_VOICE_DIR}'):
    os.mkdir(f'{APP_IMAGE_DIR}/{TARGET_VOICE_DIR}')
if not os.path.exists(f'{APP_IMAGE_DIR}/{OTHER_VOICE_DIR}') and  not os.path.isdir(f'{APP_IMAGE_DIR}/{OTHER_VOICE_DIR}'):
...
for index in range(len(df)):
    try:
        ...
        dur_aud = AudioUtil.pad_trunc(noise_aud, duration)
        shift_aud = AudioUtil.time_shift(dur_aud, shift_pct)
        
        sgram = AudioUtil.spectro_gram(shift_aud, n_mels=80, n_fft=2048, hop_len=12)
        ...
        spectrogram=plot_spectrogram(aug_sgram,title=file_name, file_name=spectrogram_file_name)
