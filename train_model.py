...
class AudioClassifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(
            in_channels=3, out_channels=8, kernel_size=5, stride=(2, 2), padding=(2, 2)
        )
        self.bn1 = nn.BatchNorm2d(8)

        self.conv2 = nn.Conv2d(
            in_channels=8, out_channels=16, kernel_size=5, stride=(2, 2), padding=(2, 2)
        )
...
def training(
    ...
    learning_rate=0.001,
    MODEL="relu",
    PATH="machine_learning_model_relu.pth",
):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch == num_epochs - 1:
            write_to_file(num_epochs, avg_loss, acc, MODEL, PATH)
    print("Finished Training")


def write_to_file(epoch, loss, accuracy, MODEL, PATH):
    # Path to your CSV file
    csv_file = "models/results.csv"

    # The first row can be your header if needed
    data = [epoch, loss, accuracy, MODEL, PATH]
    ...
    with open(csv_file, "a", newline="") as file:
        ...
        if not file_exists:
            header = ["epoch", "loss", "accuracy", "model", "path"]
...
def main(MODEL, PATH):
    ...
    num_epochs = (
        15  # increase num of epochs until there isn't much change in validation loss
    )
    learning_rate = 0.001

    training(myModel, train_dl, num_epochs, learning_rate, MODEL, PATH)
